...
class Ayum:
    ...
    def eat_food(self, f):
        if self.hunger <= 0:
            ...
            self.health_limit += random.randint(
                min(20, f.energy * 2), max(30, f.energy * 3)
            )
            self.increase_health(
                random.randint(min(20, f.energy * 2), max(30, f.energy * 3))
            )
    ...
    def hunt(self, animal):
        ...
        attack_power = random.randint(
            int(self.attack * (1.6)) + self.age * 6 - min(int(self.hunger / 4), 10),
            int(self.attack * 3.5)
            + self.age * 6
            - min(25, int((self.sleep_limit - self.sleep) * 1.5))
            - min(int(self.hunger / 4), 10),
        )
        if self.sleep > 0:
    ...
    def play_games(self):
        print(
            Fore.CYAN + "It seems Ayum wants to chill. Which game does he want to play?"
        )
    ...
    def load_state(self):
        ...
        if game_state:
            ...
            self.food_inventory = [
                Food.food.get_food_by_name(name)
                for name in game_state["food_inventory"]
                if name is not None
            ]
